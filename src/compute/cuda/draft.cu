...
///
#define NOT_ZERO(A) copysignf(fmax(LO_EPSILON, fabs(A)), (A))
///
#define NORMALIZE(V) \
{ \
...
{ \
  ...
  float p1 = r * cos(phi); \
  float p2 = r * sin(phi) * ((RYF < a) ? 1.0f : stretched[2]); \
  N[0] = ROUGHNESS[0] * \
         (p1 * t1[0] + p2 * t2[0] + sqrt(1.0f - p1 * p1 - p2 * p2) * stretched[0]); \
  N[1] = ROUGHNESS[1] * \
         (p1 * t1[1] + p2 * t2[1] + sqrt(1.0f - p1 * p1 - p2 * p2) * stretched[1]); \
  N[2] = (p1 * t1[2] + p2 * t2[2] + sqrt(1.0f - p1 * p1 - p2 * p2) * stretched[2]); \
  NORMALIZE(N); \
}
///
...
{ \
  ...
  if ((ROUGHNESS[0] == ROUGHNESS[1]) || (inv_sin_theta_2 <= 0.0f)) { \
    ...
    R = sqrt(cos_phi_2 * ROUGHNESS[0] * ROUGHNESS[0] + \
...
{ \
  if (fabs(A) > fabs(B)) { \
    R = B / A; \
    R = fabs(A) * sqrt(1.0f + R * R); \
  } else if (B != 0.0f) { \
    R = A / B; \
    R = fabs(B) * sqrt(1.0f + R * R); \
  } else {\
    R = 0.0f; \
  } \
}
...
{ \
  ...
  D[0] = r * cos_phi; \
  D[1] = r * sin_phi; \
  D[2] = sqrt(fmax(0.0f, 1.0f - D[0] * D[0] - D[1] * D[1])); \
  NORMALIZE(D); \
}
///
...
{\
  ...
  if (use_triangles_image) { \
    T[0] = w * tex1Dfetch<float>(triangles_image, TRI_VT_0) + \
           u * tex1Dfetch<float>(triangles_image, TRI_VT_1) + \
           v * tex1Dfetch<float>(triangles_image, TRI_VT_2); \
    T[1] = w * tex1Dfetch<float>(triangles_image, TRI_VT_0 + 1) + \
           u * tex1Dfetch<float>(triangles_image, TRI_VT_1 + 1) + \
           v * tex1Dfetch<float>(triangles_image, TRI_VT_2 + 1); \
    T[2] = w * tex1Dfetch<float>(triangles_image, TRI_VT_0 + 2) + \
           u * tex1Dfetch<float>(triangles_image, TRI_VT_1 + 2) + \
           v * tex1Dfetch<float>(triangles_image, TRI_VT_2 + 2); \
    flip_binormal = w * tex1Dfetch<float>(triangles_image, TRI_VT_0 + 3) + \
                    u * tex1Dfetch<float>(triangles_image, TRI_VT_1 + 3) + \
                    v * tex1Dfetch<float>(triangles_image, TRI_VT_2 + 3); \
  } else { \
...
{ \
  ...
  if (cos_theta_t2 < 0.0f) { \
    ...
    float _cos_theta_t = sqrt(cos_theta_t2); \
...
{ \
  ...
  ood[1] = o[1] * id[1]; \
  ood[2] = o[2] * id[2]; \
  int32_t h_tri_index = NO_TRI_HIT; \
  int32_t h_material_type; \
  float h_u, h_v; \
  float h_t = FLT_MAX; \
  uint32_t top_tree_height; \
  if (USE_NODES_IMAGE) { \
    uint32_t node_pixel = (top_tree_offset << 4); \
    top_tree_height = tex1Dfetch<uint32_t>(nodes_image,  \
                                           NODE_EXTRA_0); \
  } else { \
    top_tree_height = nodes[top_tree_offset].extra[0]; \
  } \
  uint32_t tree_height = top_tree_height; \
  uint32_t tree_offset = top_tree_offset; \
  uint32_t node_offset = 0; \
  uint32_t trail = 0x0; \
  uint32_t key = 0x0; \
  uint32_t level_bit = 0x1 << (top_tree_height - 1); \
  uint32_t hit_leaf = TREE_NO_LEAF_HIT; \
  uint32_t mesh_triangle_offset = 0; \
  uint32_t hit_mask = HM_TEST_NODES; \
  while (true) { \
    while (hit_leaf >= TREE_NO_LEAF_HIT) { \
      uint32_t node_index = node_offset + tree_offset; \
      uint32_t node_pixel = (node_index << 4); \
      if (level_bit == LB_NEW_SUB_TREE) { \
        if (USE_NODES_IMAGE) { \
          tree_height = tex1Dfetch<uint32_t>(nodes_image,  \
                                             NODE_EXTRA_0); \
          mesh_triangle_offset = tex1Dfetch<uint32_t>(nodes_image,  \
                                                      NODE_EXTRA_1); \
        } else { \
      ...
      if (hit_mask == HM_TEST_NODES) { \
        ...
        if (USE_NODES_IMAGE) { \
          l_bounds[0] = tex1Dfetch<float>(nodes_image, NODE_L_BOUNDS); \
          l_bounds[1] = tex1Dfetch<float>(nodes_image, NODE_L_BOUNDS + 1); \
          l_bounds[2] = tex1Dfetch<float>(nodes_image, NODE_L_BOUNDS + 2); \
          l_bounds[3] = tex1Dfetch<float>(nodes_image, NODE_L_BOUNDS + 3); \
          l_bounds[4] = tex1Dfetch<float>(nodes_image, NODE_L_BOUNDS + 4); \
          l_bounds[5] = tex1Dfetch<float>(nodes_image, NODE_L_BOUNDS + 5); \
          r_bounds[0] = tex1Dfetch<float>(nodes_image, NODE_R_BOUNDS); \
          r_bounds[1] = tex1Dfetch<float>(nodes_image, NODE_R_BOUNDS + 1); \
          r_bounds[2] = tex1Dfetch<float>(nodes_image, NODE_R_BOUNDS + 2); \
          r_bounds[3] = tex1Dfetch<float>(nodes_image, NODE_R_BOUNDS + 3); \
          r_bounds[4] = tex1Dfetch<float>(nodes_image, NODE_R_BOUNDS + 4); \
          r_bounds[5] = tex1Dfetch<float>(nodes_image, NODE_R_BOUNDS + 5); \
        } else { \
      ...
      uint child = (hit_mask & 0x1); \
      if (USE_NODES_IMAGE) { \
        child = tex1Dfetch<uint32_t>(nodes_image, NODE_L + child); \
      } else { \
    ...
    if (hit_leaf != TREE_NO_LEAF_HIT) { \
      ...
      for (uint32_t ti = tri_offset; ti <= tri_offset + tri_count; ++ti) { \
        uint32_t tri_pixel = ti * 42; \
        float vertex[3]; \
        float edges[2][3]; \
        float surface_normal[3]; \
        if (USE_TRIANGLES_IMAGE) { \
          vertex[0] = tex1Dfetch<float>(triangles_image, TRI_VERTEX); \
          vertex[1] = tex1Dfetch<float>(triangles_image, TRI_VERTEX + 1); \
          vertex[2] = tex1Dfetch<float>(triangles_image, TRI_VERTEX + 2); \
          edges[0][0] = tex1Dfetch<float>(triangles_image, TRI_EDGES_0); \
          edges[0][1] = tex1Dfetch<float>(triangles_image, TRI_EDGES_0 + 1); \
          edges[0][2] = tex1Dfetch<float>(triangles_image, TRI_EDGES_0 + 2); \
          edges[1][0] = tex1Dfetch<float>(triangles_image, TRI_EDGES_1); \
          edges[1][1] = tex1Dfetch<float>(triangles_image, TRI_EDGES_1 + 1); \
          edges[1][2] = tex1Dfetch<float>(triangles_image, TRI_EDGES_1 + 2); \
          surface_normal[0] = tex1Dfetch<float>(triangles_image, \
                                                    TRI_SURF_N); \
          surface_normal[1] = tex1Dfetch<float>(triangles_image, \
                                                    TRI_SURF_N + 1); \
          surface_normal[2] = tex1Dfetch<float>(triangles_image, \
                                                    TRI_SURF_N + 2); \
        } else { \
        ...
        float sign_dot = copysignf(1.0f, -dot); \
        ...
        if ((dot != 0.0f) && (u >= 0.0f) && (v >= 0.0f) && \
          ...
          float vertex_uvs[3][2]; \
          if (USE_TRIANGLES_IMAGE) { \
            material_id = tex1Dfetch<uint32_t>(triangles_image, TRI_MAT_ID); \
            vertex_uvs[0][0] = tex1Dfetch<float>(triangles_image, TRI_UV_0); \
            vertex_uvs[0][1] = tex1Dfetch<float>(triangles_image, TRI_UV_0 + 1); \
            vertex_uvs[1][0] = tex1Dfetch<float>(triangles_image, TRI_UV_1); \
            vertex_uvs[1][1] = tex1Dfetch<float>(triangles_image, TRI_UV_1 + 1); \
            vertex_uvs[2][0] = tex1Dfetch<float>(triangles_image, TRI_UV_2); \
            vertex_uvs[2][1] = tex1Dfetch<float>(triangles_image, TRI_UV_2 + 1); \
          } else {\
          ...
          if ((materials[material_id].glass > 0.0f) || \
            ...
            if (materials[material_id].map_flags & MAP_FLAG_EMISSION) { \
              ...
              emission_uv.y = uv[1]; \
              emission_uv.x = \
                lerp(materials[material_id].emission_bounds[0] + 0.5f, \
                     materials[material_id].emission_bounds[1] - 0.5f, \
                     emission_uv.x); \
              emission_uv.y = \
                lerp(materials[material_id].emission_bounds[2] + 0.5f, \
                     materials[material_id].emission_bounds[3] - 0.5f, \
                     emission_uv.y); \
              float4 emission_map = tex2D<float4>(emission_atlas, emission_uv.x, \
                                                  emission_uv.y); \
              emission *= fmax(fmax(emission_map.y, emission_map.z), \
                               emission_map.w); \
            } \
            if (emission == 0.0f) { \
              ...
              if (materials[material_id].map_flags & MAP_FLAG_ALBEDO) { \
                ...
                albedo_uv.y = uv[1]; \
                albedo_uv.x = \
                  lerp(materials[material_id].albedo_bounds[0] + 0.5f, \
                       materials[material_id].albedo_bounds[1] - 0.5f, \
                       albedo_uv.x); \
                albedo_uv.y = \
                  lerp(materials[material_id].albedo_bounds[2] + 0.5f, \
                       materials[material_id].albedo_bounds[3] - 0.5f, \
                       albedo_uv.y); \
                float4 albedo_map = tex2D<float4>(albedo_atlas, albedo_uv.x, \
                                                  albedo_uv.y); \
                alpha *= albedo_map.x; \
              } \
            } \
    ...
    uint32_t shift = 31 - __clz(trail & -trail); \
    ...
    uint32_t local_key = key & (0xFFFFFFFF >> (32 - tree_height)); \
    node_offset = local_key + ((32 - __popc(local_key)) - \
    ...
    if (hit_leaf != TREE_NO_LEAF_HIT) { \
      ...
      for (uint32_t ti = tri_offset; ti <= tri_offset + tri_count; ++ti) { \
        ...
        if (USE_TRIANGLES_IMAGE) { \
          ...
          edges[1][1] = tex1Dfetch<float>(triangles_image, TRI_EDGES_1 + 1); \
          edges[1][2] = tex1Dfetch<float>(triangles_image, TRI_EDGES_1 + 2); \
          surface_normal[0] = tex1Dfetch<float>(triangles_image, TRI_SURF_N); \
          surface_normal[1] = tex1Dfetch<float>(triangles_image, TRI_SURF_N + 1); \
          surface_normal[2] = tex1Dfetch<float>(triangles_image, TRI_SURF_N + 2); \
        } else { \
          vertex[0] = triangles[ti].vertex[0]; \
          vertex[1] = triangles[ti].vertex[1]; \
          vertex[2] = triangles[ti].vertex[2]; \
...
///
__global__ void Primary(float* ray_state, 
                        Material* materials,
                        int32_t use_nodes_image,
                        cudaTextureObject_t nodes_image,
                        TwoNode* nodes,
                        int32_t top_tree_offset,
                        int32_t use_triangles_image,
                        cudaTextureObject_t triangles_image,
                        TreeTriangle* triangles,
                        cudaTextureObject_t albedo_atlas,
                        cudaTextureObject_t emission_atlas,
                        Camera camera,
                        uint4  b_seeds,
                        int32_t width, 
                        int32_t height, 
                        int32_t invert_y,
												int32_t sample)
{
  int32_t global_id = GET_GLOBAL_ID();
  int32_t ray_count = width * height;
  ...
  int32_t x_pixel = global_id % width;
  int32_t y_pixel = global_id / width;
  if (invert_y) {
...
///
__global__ void Secondary(float* ray_state, 
                          Material* materials,
                          int32_t use_nodes_image,
                          cudaTextureObject_t nodes_image,
                          TwoNode* nodes,
...
///
__global__ void EnvSky(float* ray_state,
                       cudaTextureObject_t sun_table,
                       Sun sun,
											 int32_t width,
										   int32_t height,
                       int32_t bounce)
{
  ...
  int32_t state = ray_state[RS_STATE];
  ...
  if ((rmu < 0.0f) && (delta > 0.0f)) {
    cst = make_float4(1.0f, 0.0f, 0.0f, 0.484375f);
  } else {
    cst = make_float4(-1.0f, 766800.000001f, 875.67117116f, 0.515625f);
  }
  float umu = cst.w + (rmu * cst.x + sqrt(delta + cst.y)) / 
  ...
  float2 uv0 = make_float2((unu + umus) * 0.125f, umu);
  float2 uv1 = make_float2((unu + umus + 1.0f) * 0.125f, umu);
  float4 raymie = fmax((tex2D<float4>(sun_table, uv0.x, uv0.y) * 
                        (1.0f - lerp_value)) + 
                       (tex2D<float4>(sun_table, uv1.x, uv1.y) * lerp_value), 
                       make_float4(0.0f));
  float pr = 0.05968310365f * (1.0f + nu * nu);
  float pm = 0.04297183463f * pow(1.64f - 1.6f * nu, -1.5f) * 
             (1.0f + nu * nu) * 0.378378378f;
  float4 beta_r = make_float4(0.0058f, 0.0135f, 0.0331f, 1.0f);
  float4 mie = raymie * raymie.w / (raymie.x) * (beta_r.x / beta_r) * \
                (1.0f + (d_i[1] * 3.0f));
  float4 result = fmax(raymie * pr + mie * pm, make_float4(0.0f)) * 
  ...
  if ((brdf_pdf != -1.0f) && (sun_delta >= SUN_COS_THETA)) {
    ...
    color[0] = lerp(color[0], max_component * 1.0f, horizon_bias);
    color[1] = lerp(color[1], max_component * 0.98f, horizon_bias); 
    color[2] = lerp(color[2], max_component * 0.73f, horizon_bias); 
    color[0] *= sun.color[0];
    color[1] *= sun.color[1];
    color[2] *= sun.color[2];
  }
...
///
__global__ void EnvMap(float* ray_state,
                       ...
                       int bounce)
{
  ...
  d_i[2] = ray_state[RS_DIR_Z];
  NORMALIZE(d_i);
  float2 uv = make_float2(1.0f + atan2(d_i[0], -d_i[2]), acos(-d_i[1]) * INV_PI);
  uv.x *= INV_TAU;
  uv.x += environment_properties.rotation;
	float4 color = tex2D<float4>(environment_map, uv.x, uv.y);
...
///
__global__ void Emission(float* ray_state,
                         Material* materials,
                         int32_t use_triangles_image,
                         cudaTextureObject_t triangles_image,
                         TreeTriangle* triangles,
                         ...
                         int bounce)
{
  ...
  uint32_t tri_pixel = tri_index * 42;
  float vn[3];
  if (use_triangles_image) {
    vn[0] = w * tex1Dfetch<float>(triangles_image, TRI_VN_0) + 
            u * tex1Dfetch<float>(triangles_image, TRI_VN_1) + 
            v * tex1Dfetch<float>(triangles_image, TRI_VN_2);
    vn[1] = w * tex1Dfetch<float>(triangles_image, TRI_VN_0 + 1) + 
            u * tex1Dfetch<float>(triangles_image, TRI_VN_1 + 1) + 
            v * tex1Dfetch<float>(triangles_image, TRI_VN_2 + 1);
    vn[2] = w * tex1Dfetch<float>(triangles_image, TRI_VN_0 + 2) + 
            u * tex1Dfetch<float>(triangles_image, TRI_VN_1 + 2) + 
            v * tex1Dfetch<float>(triangles_image, TRI_VN_2 + 2);
  } else {
  ...
  if (dot_ni <= 0.0f) {
    ...
    float inverse_area;
    if (use_triangles_image) {
      mat_id = tex1Dfetch<uint32_t>(triangles_image, TRI_MAT_ID);
      uv[0] = w * tex1Dfetch<float>(triangles_image, TRI_UV_0) +
              u * tex1Dfetch<float>(triangles_image, TRI_UV_1) +
              v * tex1Dfetch<float>(triangles_image, TRI_UV_2);
      uv[1] = w * tex1Dfetch<float>(triangles_image, TRI_UV_0 + 1) +
              u * tex1Dfetch<float>(triangles_image, TRI_UV_1 + 1) +
              v * tex1Dfetch<float>(triangles_image, TRI_UV_2 + 1);
      inverse_area = tex1Dfetch<float>(triangles_image, TRI_INV_AREA);
    } else {
    ...
    if (materials[mat_id].map_flags & MAP_FLAG_EMISSION) {
      ...
      emission_uv.x = uv[0];
      emission_uv.y = uv[1];
      emission_uv.x = lerp(materials[mat_id].emission_bounds[0] + 0.5f,
                           materials[mat_id].emission_bounds[1] - 0.5f, 
                           emission_uv.x);
      emission_uv.y = lerp(materials[mat_id].emission_bounds[2] + 0.5f,
                           materials[mat_id].emission_bounds[3] - 0.5f, 
                           emission_uv.y);
      float4 emission_map = tex2D<float4>(emission_atlas, emission_uv.x, 
                                          emission_uv.y);
      emitter_color[0] = emission_map.y * intensity;
      emitter_color[1] = emission_map.z * intensity;
      emitter_color[2] = emission_map.w * intensity;
    } else {
...
///
__global__ void Sample(float* ray_state,
                       Material* materials,
                       int32_t use_triangles_image,
                       cudaTextureObject_t triangles_image,
                       TreeTriangle* triangles,
                       ...
                       cudaTextureObject_t metallic_atlas,
                       cudaTextureObject_t emission_atlas,
                       cudaTextureObject_t sun_table,
                       Sun sun,
                       cudaTextureObject_t environment_map,
                       ...
                       int32_t sample_sun)
{
  ...
  bool glass_material;
  {
    ...
    float v = ray_state[RS_V];
    float w = 1.0f - u - v;
    uint32_t tri_index = ray_state[RS_TRI_INDEX];
    uint32_t tri_pixel = tri_index * 42;
    uint32_t mat_id;
    float uv[2];
    if (use_triangles_image) {
      mat_id = tex1Dfetch<uint32_t>(triangles_image, TRI_MAT_ID);
      sn[0] = tex1Dfetch<float>(triangles_image, TRI_SURF_N);
      sn[1] = tex1Dfetch<float>(triangles_image, TRI_SURF_N + 1);
      sn[2] = tex1Dfetch<float>(triangles_image, TRI_SURF_N + 2);
      uv[0] = w * tex1Dfetch<float>(triangles_image, TRI_UV_0) +
              u * tex1Dfetch<float>(triangles_image, TRI_UV_1) +
              v * tex1Dfetch<float>(triangles_image, TRI_UV_2);
      uv[1] = w * tex1Dfetch<float>(triangles_image, TRI_UV_0 + 1) +
    ...
    if (materials[mat_id].map_flags & MAP_FLAG_ALBEDO) {
      ...
      albedo_uv.x = uv[0];
      albedo_uv.y = uv[1];
      albedo_uv.x = lerp(materials[mat_id].albedo_bounds[0] + 0.5f,
                         materials[mat_id].albedo_bounds[1] - 0.5f, 
                         albedo_uv.x);
      albedo_uv.y = lerp(materials[mat_id].albedo_bounds[2] + 0.5f,
                         materials[mat_id].albedo_bounds[3] - 0.5f, 
                         albedo_uv.y);
      float4 albedo_map = tex2D<float4>(albedo_atlas, albedo_uv.x, 
                                        albedo_uv.y);
      albedo[0] *= albedo_map.y;
      albedo[1] *= albedo_map.z;
      albedo[2] *= albedo_map.w;
    }
    if (materials[mat_id].map_flags & MAP_FLAG_NORMAL) {
      float2 normal_uv;
      normal_uv.x = uv[0];
      normal_uv.y = uv[1];
      normal_uv.x = lerp(materials[mat_id].normal_bounds[0] + 0.5f,
                         materials[mat_id].normal_bounds[1] - 0.5f, 
                         normal_uv.x);
      normal_uv.y = lerp(materials[mat_id].normal_bounds[2] + 0.5f,
                         materials[mat_id].normal_bounds[3] - 0.5f, 
                         normal_uv.y);
      float4 normal_map = tex2D<float4>(normal_atlas, 
                                        normal_uv.x, normal_uv.y);
      float vn_tangent[3], vn_binormal[3];
      GET_VT_BASIS(vn, vn_tangent, vn_binormal);
      float ln[3];
      ln[0] = normal_map.x * 2.0f - 1.0f;
      ln[1] = normal_map.z * 2.0f - 1.0f;
      ln[2] = sqrt(1.0f - saturate(ln[0] * ln[0] + ln[1] * ln[1]));
    ...
    if (materials[mat_id].map_flags & MAP_FLAG_METALLIC) {
      ...
      metallic_uv.x = uv[0];
      metallic_uv.y = uv[1];
      metallic_uv.x = lerp(materials[mat_id].metallic_bounds[0] + 0.5f,
                           materials[mat_id].metallic_bounds[1] - 0.5f, 
                           metallic_uv.x);
      metallic_uv.y = lerp(materials[mat_id].metallic_bounds[2] + 0.5f,
                           materials[mat_id].metallic_bounds[3] - 0.5f, 
                           metallic_uv.y);
      float4 metallic_map = tex2D<float4>(metallic_atlas, 
                                          metallic_uv.x, metallic_uv.y);
      metallic *= metallic_map.y;
      smoothness *= metallic_map.x;
    }
  ...
  if (sample_sun > 0) {
    ...
    float sun_ld_o[3];
    sun_ld_o[0] = z2 * cos(theta);
    sun_ld_o[1] = z2 * sin(theta);
    sun_ld_o[2] = z;
    float sun_tangent[3];
    ...
    if (cos_at_tri <= 0.0f) {
      ...
      float4 sun_result = fmax(raymie * pr + mie * pm, make_float4(0.0f)) * 
                          30.0f * sun.sky_intensity;
      ...
      sun_color[0] = lerp(sun_color[0], max_component * 1.0f, horizon_bias);
      sun_color[1] = lerp(sun_color[1], max_component * 0.98f, horizon_bias); 
      sun_color[2] = lerp(sun_color[2], max_component * 0.73f, horizon_bias); 
      float ld_o[3];
      ...
      PRNG(global_id, b_seeds_2.x, sample_dice);
			if (glass_material || (sample_dice >= metallic)) {
        // Dielectric interface
        bool refract = glass_material && ((ld_i[2] * ld_o[2]) < 0.0f);
        if (refract) {
          // Refraction facet
          ...
          if (glass_material || (reflect_sample <= (fresnel * coating_dropoff))) {
            ...
            if ((last_brdf_pdf == DIRAC_PDF) && (smoothness < 1.0f)) {
              if ((d == 0.0f) || (ld_i[2] == 0.0f)) {
                ...
                float model = fresnel * coating_dropoff * d * g / 
                              (4.0f * fabs(ld_i[2]));
                brdf_color[0] = model;
                brdf_color[1] = model;
                brdf_color[2] = model;
  ...
  if (emitter_count > 0) {
    ...
    uint32_t emitter_tri_index = emitter_index.mesh_triangle_offset + 
                                 emitter_index.triangle_offset;
    uint32_t tri_pixel = emitter_tri_index * 42;
    float emitter_position[3];
    if (use_triangles_image) {
      emitter_position[0] = tex1Dfetch<float>(triangles_image, TRI_VERTEX) +
        (emit_u * tex1Dfetch<float>(triangles_image, TRI_EDGES_0)) +
        (emit_v * -tex1Dfetch<float>(triangles_image, TRI_EDGES_1));
      emitter_position[1] = tex1Dfetch<float>(triangles_image, TRI_VERTEX + 1) +
        (emit_u * tex1Dfetch<float>(triangles_image, TRI_EDGES_0 + 1)) +
        (emit_v * -tex1Dfetch<float>(triangles_image, TRI_EDGES_1 + 1));
      emitter_position[2] = tex1Dfetch<float>(triangles_image, TRI_VERTEX + 2) +
        (emit_u * tex1Dfetch<float>(triangles_image, TRI_EDGES_0 + 2)) +
        (emit_v * -tex1Dfetch<float>(triangles_image, TRI_EDGES_1 + 2));
    } else {
    ...
    if (use_triangles_image) {
      mat_id = tex1Dfetch<uint32_t>(triangles_image, TRI_MAT_ID);
      inverse_area = tex1Dfetch<float>(triangles_image, TRI_INV_AREA);
      emitter_n[0] = emit_w * tex1Dfetch<float>(triangles_image, TRI_VN_0) + 
                     emit_u * tex1Dfetch<float>(triangles_image, TRI_VN_1) +  
                     emit_v * tex1Dfetch<float>(triangles_image, TRI_VN_2);
      emitter_n[1] = emit_w * tex1Dfetch<float>(triangles_image, TRI_VN_0 + 1) + 
                     emit_u * tex1Dfetch<float>(triangles_image, TRI_VN_1 + 1) +  
                     emit_v * tex1Dfetch<float>(triangles_image, TRI_VN_2 + 1);
      emitter_n[2] = emit_w * tex1Dfetch<float>(triangles_image, TRI_VN_0 + 2) + 
                     emit_u * tex1Dfetch<float>(triangles_image, TRI_VN_1 + 2) +  
                     emit_v * tex1Dfetch<float>(triangles_image, TRI_VN_2 + 2);
    } else {
    ...
    if ((cos_at_emitter <= 0.0f) || (cos_at_tri <= 0.0f) || 
      ...
      if (materials[mat_id].map_flags & MAP_FLAG_EMISSION) {
        float2 emission_uv;
        if (use_triangles_image) {
          emission_uv.x = emit_w * tex1Dfetch<float>(triangles_image, TRI_UV_0) +
                          emit_u * tex1Dfetch<float>(triangles_image, TRI_UV_1) +
                          emit_v * tex1Dfetch<float>(triangles_image, TRI_UV_2);
          emission_uv.y = emit_w * tex1Dfetch<float>(triangles_image, TRI_UV_0 + 1) +
                          emit_u * tex1Dfetch<float>(triangles_image, TRI_UV_1 + 1) +
                          emit_v * tex1Dfetch<float>(triangles_image, TRI_UV_2 + 1);
        } else {
...
///
__global__ void NEE(float* ray_state, 
                    Material* materials,
                    int32_t use_nodes_image,
                    cudaTextureObject_t nodes_image,
                    TwoNode* nodes,
...
///
__global__ void Accumulate(float* ray_state,
                           ...
                           float inv_sample_count)
{
  ...
  float lum_coeff[3];
	...
	float sat_color[3];
	sat_color[0] = lerp(intensity, bright_rgb[0], image_properties.saturation);
	sat_color[1] = lerp(intensity, bright_rgb[1], image_properties.saturation);
	sat_color[2] = lerp(intensity, bright_rgb[2], image_properties.saturation);
  rgb[0] = lerp(0.5f, sat_color[0], image_properties.contrast);
	rgb[1] = lerp(0.5f, sat_color[1], image_properties.contrast);
	rgb[2] = lerp(0.5f, sat_color[2], image_properties.contrast);
  uchar4 target_color = make_uchar4(saturate(rgb[0]) * 255.0f,
                                    saturate(rgb[1]) * 255.0f,
                                    saturate(rgb[2]) * 255.0f, 255); 
  int32_t x = global_id % width;
  int32_t y = global_id / width;
  surf2Dwrite(target_color, target, x * sizeof(uchar4), y, 
              cudaBoundaryModeClamp);
}
///
__global__ void CompositeTwo(cudaSurfaceObject_t compositing_target_one,
                             cudaTextureObject_t compositing_target_two,
                             cudaSurfaceObject_t target,
                             int32_t width, 
                             int32_t height)
{
  ...
  int32_t x = global_id % width;
  int32_t y = global_id / width;
  uint4 c0 = make_uint4(surf2Dread<uchar4>(compositing_target_one, 
                                           x * sizeof(uchar4), y,
                                           cudaBoundaryModeClamp));
  uint4 c1 = make_uint4(tex2D<uchar4>(compositing_target_two, x, y));
  uint4 color = (c0 + c1) / make_uint4(2);
  uchar4 target_color = make_uchar4(color, 255); 
  surf2Dwrite(target_color, target, x * sizeof(uchar4), y, 
              cudaBoundaryModeClamp);
}
///
__global__ void CompositeThree(cudaSurfaceObject_t compositing_target_one,
                               cudaTextureObject_t compositing_target_two,
                               cudaTextureObject_t compositing_target_three,
                               cudaSurfaceObject_t target,
                               int32_t width, 
                               int32_t height)
{
  ...
  uint4 c0 = make_uint4(surf2Dread<uchar4>(compositing_target_one, 
                                           ...
                                           cudaBoundaryModeClamp));
  uint4 c1 = make_uint4(tex2D<uchar4>(compositing_target_two, x, y));
  uint4 c2 = make_uint4(tex2D<uchar4>(compositing_target_three, x, y));
  uint4 color = (c0 + c1 + c2) / make_uint4(3);
  uchar4 target_color = make_uchar4(color, 255); 
  surf2Dwrite(target_color, target, x * sizeof(uchar4), y, 
              cudaBoundaryModeClamp);
}
///
__global__ void CompositeFour(cudaSurfaceObject_t compositing_target_one,
                              cudaTextureObject_t compositing_target_two,
                              cudaTextureObject_t compositing_target_three,
                              cudaTextureObject_t compositing_target_four,
                              cudaSurfaceObject_t target,
                              int32_t width, 
                              int32_t height)
{
  ...
  uint4 c1 = make_uint4(tex2D<uchar4>(compositing_target_two, x, y));
  uint4 c2 = make_uint4(tex2D<uchar4>(compositing_target_three, x, y));
  uint4 c3 = make_uint4(tex2D<uchar4>(compositing_target_four, x, y));
  uint4 color = (c0 + c1 + c2 + c3) / make_uint4(4);
  uchar4 target_color = make_uchar4(color, 255); 
  surf2Dwrite(target_color, target, x * sizeof(uchar4), y, 
              cudaBoundaryModeClamp);
}
///
__global__ void CompositeFive(cudaSurfaceObject_t compositing_target_one,
                              cudaTextureObject_t compositing_target_two,
                              cudaTextureObject_t compositing_target_three,
                              cudaTextureObject_t compositing_target_four,
                              cudaTextureObject_t compositing_target_five,
                              cudaSurfaceObject_t target,
                              int32_t width, 
                              int32_t height)
{
  ...
  uint4 c2 = make_uint4(tex2D<uchar4>(compositing_target_three, x, y));
  uint4 c3 = make_uint4(tex2D<uchar4>(compositing_target_four, x, y));
  uint4 c4 = make_uint4(tex2D<uchar4>(compositing_target_five, x, y));
  uint4 color = (c0 + c1 + c2 + c3 + c4) / make_uint4(5);
  uchar4 target_color = make_uchar4(color, 255); 
  surf2Dwrite(target_color, target, x * sizeof(uchar4), y, 
              cudaBoundaryModeClamp);
}
